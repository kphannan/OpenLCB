...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, ExtCtrls, SynMemo, StrUtils;
...
var
  s1: string;
begin
  MemoACDI.Lines.BeginUpdate;
  try
    s1 := '';
    MemoACDI.Lines.Clear;
    s1 := '';
    s1 := 'const';
    MemoACDI.Lines.Add(s1);
    if CheckBoxACDI.Checked then
    begin
      MemoACDI.Lines.Add('  MAX_ACDI_ARRAY_VNODE = ' + IntToStr(ACDILen)+';');
      s1 := '  ACDI_VNODE_STRINGS: array[0..MAX_ACDI_ARRAY_VNODE - 1] of byte = (';
    end else
    begin
       MemoACDI.Lines.Add('  MAX_ACDI_ARRAY = ' + IntToStr(ACDILen)+';');
      s1 := '  ACDI_NODE_STRINGS: array[0..MAX_ACDI_ARRAY - 1] of byte = (';
    end;
    MemoACDI.Lines.Add(s1);
    MemoACDI.Lines.Add(PackString(EditMfgInfoVer.Text, True, True));
    MemoACDI.Lines.Add(PackString(EditMfg.Text, True, False));
    MemoACDI.Lines.Add(PackString(EditName.Text, True, False));
    MemoACDI.Lines.Add(PackString(EditHWVersion.Text, True, False));
    MemoACDI.Lines.Add(PackString(EditSWVersion.Text, True, False));
    MemoACDI.Lines.Add(PackString(EditUserInfoVer.Text, True, True));
    MemoACDI.Lines.Add(PackString(EditUserDefinedName.Text, True, False));
    MemoACDI.Lines.Add(PackString(EditUserDefinedDesc.Text, False, False));
    s1 := '    );';
    MemoACDI.Lines.Add(s1)
  finally
    MemoACDI.Lines.EndUpdate
  end;
end;
...
var
  Index: Integer;
begin
  if Pos('const', LowerCase(MemoACDI.Lines[0])) = 0 then
    Index := 2
  else
    Index := 3;
  EditMfgInfoVer.Text := DecompileVersionString(MemoACDI.Lines[Index]);
  Inc(Index);
  EditMfg.Text := DecompileString(MemoACDI.Lines[Index]);
  Inc(Index);
  EditName.Text := DecompileString(MemoACDI.Lines[Index]);
  Inc(Index);
  EditHWVersion.Text := DecompileString(MemoACDI.Lines[Index]);
  Inc(Index);
  EditSWVersion.Text := DecompileString(MemoACDI.Lines[Index]);
  Inc(Index);
  EditUserInfoVer.Text := DecompileVersionString(MemoACDI.Lines[Index]);
  Inc(Index);
  EditUserDefinedName.Text := DecompileString(MemoACDI.Lines[Index]);
  Inc(Index);
  EditUserDefinedDesc.Text := DecompileString(MemoACDI.Lines[Index]);
end;
