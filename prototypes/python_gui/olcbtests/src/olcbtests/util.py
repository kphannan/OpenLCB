...
def is_nodetest(obj):
    ...
    * ``conn`` is an instance of 
      :py:class:`openlcb.communication.EthernetConnection`
...
def get_tests():
    ...
    contents = pkg_resources.resource_listdir(pkg_name, None)
    mod_suffixes = [s.rsplit('.', 1)[-1] for s, _, _ in imp.get_suffixes()]
    modules = set()
    for item in contents:
        if not pkg_resources.resource_isdir(pkg_name, item):
            try:
...
def discover_node(conn, src_alias):
    ...
    conn.send(msg)
    r = conn.receive()
    response = messages.VerifiedNodeIDNumber.from_string(r)
