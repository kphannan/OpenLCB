...
typedef struct DccCVCache
{
    uint8_t iscached[(MAXCV+7)/8];
    uint8_t cache[MAXCV];
} DCC_CV_CACHE;
#endif
...
#else
#define cached_read_byte eeprom_read_byte
#define cached_write_byte eeprom_write_byte
#endif
uint8_t readCV( uint16_t CV )
{
  ...
  Value = cached_read_byte( (uint8_t*) CV ) ;
...
uint8_t writeCV( uint16_t CV, uint8_t Value)
{
  ...
  if( cached_read_byte( (uint8_t*) CV ) != Value )
  {
    cached_write_byte( (uint8_t*) CV, Value ) ;

    if( notifyCVChange )
      notifyCVChange( CV, Value) ;
  }

  return cached_read_byte( (uint8_t*) CV ) ;
}
