...
void can_regs_write( uint8_t address, uint8_t n, uint8_t data[] );
uint8_t can_reg_read( uint8_t address);
uint8_t can_reg_read_write( bool wr, uint8_t address, uint8_t data);
...
{
    for(int i=0;i<n;i++) {
      can_reg_write(address+i,data[i]);
    }
}
...
{
    SELECT_MCP2515;
    spi_putc( wr?SPI_WRITE:SPI_READ);
    spi_putc(address);
    data=spi_putc(data);  
    DESELECT_MCP2515;
    return data;
...
void setup()
{
  ...
  Serial.println();
  Serial.println(":I LEDuino CAN-USB Adaptor Version: DPH1.3;");
...
//  can_reg_write(0x70,0x60);
  ...
  // lets try masks and filters
  uint8_t mode = can_reg_read( CANSTAT );  // we have to change to 'config' mode
  can_change_op_mode( (1<<REQOP2) );
  uint8_t stand[4] = {0,0,0,0};
  uint8_t extnd[4] = {0,0x08,0,0};
  can_regs_write(0x20,4,stand); // mask0
  can_regs_write(0x00,4,stand); // filter0  11-bit
  can_regs_write(0x04,4,stand); // filter1  11-bit
  can_regs_write(0x24,4,extnd); // mask1
  can_regs_write(0x08,4,extnd); // filter2  29-bit extended-bit set
  can_regs_write(0x10,4,extnd); // filter3  29-bit extended-bit set
  can_regs_write(0x14,4,extnd); // filter4  29-bit extended-bit set
  can_regs_write(0x18,4,extnd); // filter5  29-bit extended-bit set
  can_reg_write(0x60,0x00);          // accept 11 or 29-bit filters
  can_reg_write(0x70,0x00);          // accept 11 or 29-bit filters
  can_change_op_mode( mode );
