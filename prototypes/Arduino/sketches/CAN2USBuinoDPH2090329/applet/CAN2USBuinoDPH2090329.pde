...
void can_reg_read( uint8_t address)
{
    uint8_t data;
    SELECT_MCP2515;
    spi_putc(SPI_WRITE);
...
void can_register_dump(void)
{
	...
	uint8_t i;
	for (i=0; i < 16; i++) {
		printf("%02x: %02x ", i, mcp2515_read_register(i));
		printf("%02x: %02x ", i+16*1, mcp2515_read_register(i+16*1));
		printf("%02x: %02x ", i+16*2, mcp2515_read_register(i+16*2));
		printf("%02x: %02x ", i+16*3, mcp2515_read_register(i+16*3));
		printf("%02x: %02x ", i+16*4, mcp2515_read_register(i+16*4));
		printf("%02x: %02x ", i+16*5, mcp2515_read_register(i+16*5));
		printf("%02x: %02x ", i+16*6, mcp2515_read_register(i+16*6));
